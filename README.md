...
7. ✅ **清理未选学科** — 未选的学科模板自动删除（提示词/记录目录/看板卡片），保持系统干净
8. ✅ 启动看板并打开浏览器
9. ✅ **设置每日自动化任务** — 弹出选项卡配置两个定时任务：
   - **每日复习提醒**（默认 9:00）— 自动检查复习计划，提醒今日到期的复习内容
   - **每日学习总结**（默认 22:00）— 总结当日学习情况，核实并更新已掌握知识点、易错点、复习计划

整个流程约 5-15 分钟（取决于选了几科），完成后直接说"切换成数学老师"开始学习。自动化任务会在后台每天按时执行，无需手动干预。
...
- **每日复习提醒**：自动化任务每天定时检查到期复习内容并主动提醒（初始化时配置，默认 9:00）
- **每日学习总结**：自动化任务每天定时总结当日学习情况，更新掌握状态/易错点/复习计划（初始化时配置，默认 22:00）
