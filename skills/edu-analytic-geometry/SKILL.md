...
agent_created: true
source: https://github.com/wy51ai/edulab (Apache-2.0)
---
...
形态与 `template/board.html` 一致。

## 依赖（重要）
计算核心 `lib/analytic_kernel.py` 依赖 **sympy**。运行脚本前先确认有能 import sympy 的
解释器：`python3 -c "import sympy"`（本项目 sympy 1.14）。

**Windows 环境**：Python 输出含中文/Unicode 时可能遇到 GBK 编码错误，运行脚本前设
`export PYTHONIOENCODING=utf-8`（Git Bash）或 `$env:PYTHONIOENCODING="utf-8"`（PowerShell）。
...
- 起本地静态服务（服务**输出文件所在目录**，即 cwd）用预览检查：无控制台报错、KaTeX 正常、
  滑块实时重算正确、范围条/定值/定点/轨迹行为符合、画笔与收起面板可用。
   （在 WorkBuddy 中用 `present_files` 直接预览生成的 HTML 文件。）

> ⚠️ **预览后清理**：如果用 `http.server` 起了临时服务，用完立即 `kill` 掉。自检产生的临时文件一律清掉。
