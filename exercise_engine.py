...
def select_from_bank(knowledge_point, count=8):
    """
    从数学题库中按知识点选题。
    先尝试精确映射，再尝试模糊匹配。
    返回题目列表，每道题为 dict。
    """
    bank_dir = QUESTION_BANK_DIR / "math" / "grade-1" / "generated"
    if not bank_dir.exists():
        return []

    all_questions = []
    for jsonl_file in sorted(bank_dir.glob("*.jsonl")):
        try:
...
def select_from_bank_all_subtypes(knowledge_point, count=8):
    """选题时尽量覆盖不同子题型。"""
    questions = select_from_bank(knowledge_point, count * 3)
...
def render_html(subject, knowledge_point, questions, paper_size="A4"):
    ...
    score_header = "".join(f"<td>{diff_names.get(d, d)}</td>" for d in section_diffs)
...
def generate_exercise(subject, count=8, paper_size="A4", topic=None):
    """
    为一个学科生成练习题 HTML。
    ...
    for i, kp in enumerate(knowledge_points):
        ...
        if subject == "math":
            qs = select_from_bank_all_subtypes(kp, n)
            if qs:
...
def main():
    ...
    print("=" * 60)
    print("  [我是学霸] 练习题生成引擎")
    print(f"  模式: 学习进度驱动 | 纸张: {args.paper} | 每题{args.count}道")
    print("=" * 60)
    ...
    for subj in subjects_to_gen:
        ...
        try:
            filepath = generate_exercise(subj, args.count, args.paper, args.topic)
            if filepath:
